...
- **New v2.0 Algorithm**: Complete rewrite for variable frequency support
  ...
  - Factor 1: Workload balance (×100 points)
  - Factor 2: Recent activity penalty (×10 points)
  - Factor 3: Household variety bonus (×500 points) - 2.5× stronger than v1.1
  - Factor 4: Recent household visit penalty (×300-1000 points) - NEW
  - Factor 5: Consecutive week prevention (×200 points) - NEW
  - Factor 6: Frequency preference (×5 points)
  - Factor 7: High-frequency household variety (×100 points) - NEW

- **Dual-Mode Support**: Automatic detection of uniform vs. variable frequency
  - Empty Column T → Uses proven v1.1 uniform frequency algorithm
  - Column T with custom frequencies → Uses new v2.0 variable frequency algorithm
...
- **QR Code Generation**: Automatic QR code for mobile access
  - 500×500 pixel PNG for high-quality printing
...
- **Renamed Menu Item**: "Archive Current Schedule" → "Generate Schedule Summary Sheet"
...
- **Automatic Mode Detection**: System chooses algorithm based on data
  ...
  - `hasCustomFrequencies = true` → v2.0 variable frequency algorithm
  - `hasCustomFrequencies = false` → v1.1 uniform frequency algorithm
...
- **Title Format**: Changed from "Deacon visits Household" to "Deacon ➡︎ Household"
...
- **Zero Breaking Changes**: All v1.1 spreadsheets work without modification
  - Empty Column T → Exact v1.1 behavior
  - No custom frequencies → v1.1 uniform algorithm used
...
- **Menu labels**: Archive → Generate Schedule Summary Sheet
