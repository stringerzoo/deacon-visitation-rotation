...
function updateContactInfoOnly() {
  /**
   * DEBUG VERSION - Updates ONLY contact information in existing calendar events
   * Added extensive logging to diagnose matching issues
   */
  try {
    const sheet = SpreadsheetApp.getActiveSheet();
    const config = getConfiguration(sheet);
    ...
    console.log('=== DEBUG: Configuration Data ===');
    console.log('Households in config:', config.households);
    console.log('Phones in config:', config.phones);
    
    // Confirm with user
    const ui = SpreadsheetApp.getUi();
    const response = ui.alert(
      'Update Contact Info Only (DEBUG)',
    ...
    console.log(`=== DEBUG: Found ${existingEvents.length} existing events ===`);
    ...
    existingEvents.forEach((event, index) => {
      try {
        // Extract household name from event title
        const title = event.getTitle();
        console.log(`\n--- DEBUG Event ${index + 1} ---`);
        console.log(`Event title: "${title}"`);
        
        // Try the regex pattern: "[Deacon] visits [Household]"
        ...
        if (visitMatch) {
          ...
          console.log(`Parsed - Deacon: "${deacon}", Household: "${household}"`);
          
          // Find household in current data
          const householdIndex = config.households.indexOf(household);
          console.log(`Household index in config: ${householdIndex}`);
          
          if (householdIndex >= 0) {
            console.log(`✅ MATCH FOUND for household: "${household}"`);
            ...
            console.log(`Current description preview: "${currentDescription.substring(0, 100)}..."`);
...
${config.calendarInstructions}`;
            ...
            console.log(`✅ UPDATED event for ${household}`);
            
            // Rate limiting - pause every 25 updates
            if ((index + 1) % 25 === 0) {
              Utilities.sleep(1000);
              console.log(`Pausing after ${index + 1} events...`);
            }
          } else {
            console.log(`❌ NO MATCH: "${household}" not found in households list`);
            console.log(`Available households:`, config.households);
          }
        } else {
          console.log(`❌ REGEX FAILED: Title "${title}" doesn't match pattern`);
        }
      } catch (eventError) {
    ...
    console.log(`\n=== DEBUG SUMMARY ===`);
    console.log(`Total events processed: ${existingEvents.length}`);
    console.log(`Events updated: ${updatedCount}`);
    console.log(`Processing time: ${new Date().getTime() - updateStartTime}ms`);
    
    ui.alert(
      'Contact Info Update Complete (DEBUG)',
